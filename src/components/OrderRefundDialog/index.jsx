...
export default function OrderRefundDialog(props) {
  ...
  async function refundOrder() {
    if (refundValue <= 0) {
      setErrorMessage('Refund value should be greater than 0');
      return;
    }
    setIsLoading(true);
    ...
    try {
      const [code, res] = await api.order.giveRefund(data);
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? <CircularProgress /> : 'Give Refund'}
          </Button>
        </DialogActions>
      </BootstrapDialog>
    </div>
