...
export default function OrderShipViewDialog(props) {
  ...
  async function shipOrder() {
    if (deliveryId.length === 0) {
      setErrorMessage('Delivery id is required');
      return;
    }
    setIsLoading(true);

    const data = {
      dispatchedDate: moment(Date.now()).format('YYYY-MM-DD'),
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? <CircularProgress /> : 'Ship'}
          </Button>
        </DialogActions>
      </BootstrapDialog>
    </div>
